...
if __name__ == '__main__':
    validation_data = pd.read_csv(r'C:\git\data\temperature_resampled\COOLING_temp_re.csv',
                       parse_dates=True, index_col=0)
    simulation_Tint_IN = pd.read_csv(r'file:///C:\git\data\temperature_resampled\calc_EER_%20Tint_IN\calc_eer_all.csv',
                         parse_dates=True, index_col=0)
    simulation_Tint_OUT = pd.read_csv(r'file:///C:\git\data\temperature_resampled\calc_EER_Tint_OUT\calc_eer_all_Tint_OUT.csv',
                         parse_dates=True, index_col=0)

    for quality_grade in simulation_Tint_IN:

        rmse_IN = get_rmse(simulation_Tint_IN['{}'.format(quality_grade)], validation_data['EER'],
                           bias_corrected=False, normalized=False)
        print("{}:".format(quality_grade),rmse_IN)

    print("")

    for quality_grade in simulation_Tint_OUT:
        rmse_OUT = get_rmse(simulation_Tint_OUT['{}'.format(quality_grade)], validation_data['EER'],
                           bias_corrected=False, normalized=False)

        print("{}:".format(quality_grade),rmse_OUT)